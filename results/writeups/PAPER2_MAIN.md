...
**A separate exploratory diagnostics note is being produced in parallel:
[`results/stage2_confirm/h7_diagnostics.md`](../stage2_confirm/h7_diagnostics.md).** It is
not a source for anything in this section; this paper does not wait on it.

---

## 4. H6 — follow-up value

> **[H6 verdict pending: classifier gate + arms running in this phase; section filled from
> the H6 report when its verdict exists.]**

No H6 number appears anywhere in this paper. The H6 parameters (token budget B, segment and
chain definitions, rich/poor thresholds, flagged-turn threshold) are the one slot the
bar-lock addendum deliberately left open, and they must be filled and owner-approved before
any confirmatory H6 scoring
([Addendum A to Amendment 2](../../PREREGISTRATION_AMENDMENT_2_ADDENDUM_A.md)). The
classifier's part-1 trust gate cleared on dev subjects (raw 0.8667, κ 0.7333, deviation D3);
part 2 — a fresh blind tranche drawn from confirmatory subjects — is binding and still to
come.
...
- **Judge canary.** A 10-row canary ran at the start of every judging session, 0 label flips
  across 2 runs, **before any confirmatory judge call was made**. The halt-on-flip rule never
  fired ([confirm report §6](../stage2_confirm/STAGE2_CONFIRM_REPORT.md)).
...
|---|---|---|---|
| generation, robustness model (flash-lite) | 1,911 | $1.676161 | 0 |
| generation, primary model (Gemma on Leonardo) | 1,911 | $0 | 0.6028 |
| stance judging | 3,822 | $4.851384 | 0 |
| judge canary | 20 | $0.025324 | 0 |
...
([`cost_log.jsonl`](../cost_log.jsonl), 92 entries): **$10.33 API** and **13.34 Leonardo
...
**Human labour was substituted by LLM auditors four times.** Deviations D1–D4 record it:
the judge audit's human tranche was 17 of 51 rows; the fuzzy-host spot-check was fully
substituted; the H6 trust audit ran as a blind LLM co-audit; the parameter-5 auditor line was
a rubric-briefed LLM ([confirm report §8](../stage2_confirm/STAGE2_CONFIRM_REPORT.md)). Each
is documented, none is pooled with a human line, and a reviewer is entitled to weight them
lower than owner labels.
...
| H7 exploratory diagnostics (in production, parallel) | [`stage2_confirm/h7_diagnostics.md`](../stage2_confirm/h7_diagnostics.md) |
